...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        uint256 masonryShare = _balances[msg.sender];
        require(masonryShare >= amount, "Masonry: withdraw request greater than staked amount");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = masonryShare.sub(amount);
...
contract Masonry is ShareWrapper, ContractGuard {
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            mry.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        mry.safeTransferFrom(msg.sender, address(this), amount);
