...
contract MRY is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the Mry ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Mry Token", "MRY") {
        // Mints 1 Mry to contract creator for initial pool setup
    ...
    function _getMryPrice() internal view returns (uint256 _mryPrice) {
        try IOracle(mryOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Mry: failed to fetch MRY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _mryPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_mryPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMryOracle(address _mryOracle) public onlyOperatorOrTaxOffice {
        require(_mryOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MRY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MRY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMryPrice = _getMryPrice();
            currentTaxRate = _updateTaxRate(currentMryPrice);
            if (currentMryPrice < burnThreshold) {
                burnTax = true;
            }
        }
